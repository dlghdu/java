...
// - 순서 보장 안됨 : 일반적으로 요소의 삽입 순서를 보장하지 않지만, 일부 구현체는 순서를 유지할 수 있다.
// - 빠른 검색 : Set은 특정 요소가 존재하는지 빠르게 검색할 수 있다.

// * 주요 메서드
// - add(E e) : 집합에 요소를 추가한다. 중보고딘 요소가 추가되면 추가되지 않는다.
// - remove(Object o) : 특정 요소를 제거한다.
// - contains(Object o) : 집합에 특정 요소가 포함되어 잇는지 확인한다.
...
public class A_set {
    ...
    // 1. HashSet
    // - 해시 기반 : 내부적으로 HashMap을 사용하여 요소를 저장한다.
    // - 빠른 기능 : 요소의 추가, 삭제, 검색 속도가 빠르다. 시간복잡도 : 0(1)
