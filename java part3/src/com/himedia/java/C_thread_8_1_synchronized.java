...
// 이처럼 한 슬데ㅡ가 진행 중인 작업을 다른 스레드가 간섭하지 못하도록 막는 것을
// '스레드의 동기화(synchronization)'라고 한다.

// * synchronized를 이용한 동기화
// 1. 메서드 전체를 임계 영역으로 지정

// -> 메서드가 호춛된 시점부터 해당 메서드가 포함된 객체의 lock을 얻어 작업을 수행하다가
...
// -> 메서드 내의 코드 일부를 블럭{}으로 감싸고 블럭 앞에 synchronized(참조 변수)를 붙이는 것인데,
// 이때 참조변수는 lock을 걸고자하는 객첼르 참조하는 것이어야 한다.
/*
...
// 가능하면 메서드 전체에 락을 거는거보다 synchronized블럭으로 임계 영역
