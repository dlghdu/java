...
// * 주요 특징
// - 키-값 쌍 : 각 요소를 키와 값의 쌍으로 저장한다.
// - 중복된 키 허용 안함 : 동일한 키를 두 번 추가할 수 없다.
// 만약 동일한 키를 다시 추가하면 기존의 값이 덮어씌워진다.
// - 빠른 검색 : 특정 키를 사용하여 값에 빠르게 접근할 수 있다.

// * 주요 메서드
// - put(K kye, V value) : 맴에 키와 값을 추가한다.
// 만약 동일한 키가 이미 존재한다면 해당 키의 값이 새 값으로 대체된다.
// - get(Object key) : 키의 대응하는 값을 반환. 해당 키가 존재하지 않으면 null을 반환.
// - remove(Object key) : 특정 키에 대응하는 키-값 쌍을 제거 한다.
...
// * 요약
// - hashmap은 빠른 검색, 삽입 성능을 제공. 요소의 순서를 보장하지 않음.
// - treemap은 키를 기준으로 정렬된 순서를 유지하며, 레드-볼래 트리 구조로 구성되어 있다.
