...
// * Queue
// 먼저 삽입된 요소가 먼저 제거되는 선입선출(FIFO : first in first out)
// Queue 인터페이스는 자바에서 큐의 동작을 정의하며, 다양한 구현체가 있다.
// 대표적으로 LinkedList, PriorityQueue, ArrayQueue 등이 있다.

// * 주요 메서드
// - add(E e) : 큐의 맨 뒤에 요소를 추가. 큐가 가득 찬 경유 예외 발생 시킴.
// - offer(E e) : 큐의 맨 뒤에 요소를 추가. 큐가 가득 차도 예외 발생 시키지 않고 false를 반환함.
// - remove() : 큐의 맨 앞에 있는 요소를 제거하고 반환함. 큐가 비어 있는 경우 예외 발생 시킴.
// - poll() : 큐의 맨 앞에 있는 요소를 제거하고 반환함. 큐가 비어 있으면 null을 반환함.
// - element() : 큐의 맨 앞에 있는 요소를 반환하지만, 제거하지는 않음. 큐가 비어 있는 경우 예외 발생 시킴.
// - peek() : 큐의 맨 앞에 있는 요소를 제거하고 반환함. 큐가 비어 있으면 null을 반환함.
...
public class A_collections_queue {
    ...
    // 1. LinkedList
    // - Queue인터페이스를 구현하므로, 큐로 사용 할 수 있다.
    // - 양방향 연결 리스트로, 큐와 리스트 모두 사용 할 수 있다.
    ...
    // 2. PriorityQueue
    // - 요소가 자연순서(기본순서, 그냥 1~10같이 순서대로)나 제공된 비교기(Comparator)에 따라 결정된다.
    // - FIFO 순서가 아니라 우선 순위에 따라 요소가 처리된다.
    public static void exam2() {
        ...
        // 순회 방법 3
        priorityQueue.forEach(System.out::println); // 람다식이라고 한다한다.
        priorityQueue.forEach(
                element -> System.out.println("Element : " + element)
        ); // 이렇게도 사용이 가능하다라는 것같다.라는 것이다.
    }
...
//        }
    ...
    // 3. ArrayQueue
    // - 큐와 덱(Deque, 양방향 큐)으로 사용할 수 있는 매우 효율적인 클래스이다.
