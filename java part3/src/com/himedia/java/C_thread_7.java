...
// TIMED_WAITING은 일시정지 시간이 지정된 경우를 의미한다.
...
// -> 지정된 일시정지 시간이 다되거나(time-out), notfy(), resume(), interrupt(0가 호출되면
...
// - interrupt() : sleep()이나 join()에 의해 일시정지인 상태인 스레드를 깨워서 실행대기상태로 만든다.
// 해당 스레드에서는 interruptedException이 발생함으로서 일시정지 상태를 벗어나게 된다.
// - stop() : 스레드를 즉시 종료시킨다.
...
//        }
        ...
        // 그 이유는 sleep()이 항상 현재 실행중인 스레드에 대해 작동하기 때문에 t1.sleep()을 하였어도
        // 실제로 영향을 받는 것은 main메서드를 실행하는 main스레드이다.
        // 그래서 sleep()은 참조변수를 이용해서 호출하기 보다 static으로 선언되어 있는
        // Thread.sleep(2000)과 같이 사용해야 된다.

    }
    
    // - itnerrupt() : 스레드의 작업을 취소한다.
    ...
    // - Thread.yield()
    // 자바의 스레드 제어 메서드 중 하나로, 현재 실행 중인 스레드가 CPU를 다른 스레드에게 양보하도록 한다.
    ...
    public static void exam4() {
        ...
        // join을 사용하지 않았다면 main 스레드가 바로 종료되었겠지만,
        // join()으로 스레드 t1, t2가 모두 작업을 마칠 때 까지 main()스레드가 기다리도록 하였다.
    }
