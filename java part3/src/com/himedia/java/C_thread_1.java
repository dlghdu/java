...
// 둘 이상의 스레드를 가진 프로세스를 '멀티 스레드 프로세서'라고 한다.
...
// 여러 사용자에게 서비스를 해주는 서버 프로그램의 경우 멀티스레드로 작성하는 것이 필수적이여서
// 하나의 서버 프로세스가 여러 개의 스레드를 생성해서
// 스레드와 사용자의 요청이 일대일로 처리되도록 프로그래밍해야 한다.
// 만일 싱글 스레드로 서버 프로그램을 작성한다면 사용자의 요청 마다 새로운 프로세스를 행성해야하는데,
...
// 작업을 하기 때문에 발생할 수 있는 동기화, 교착상태와 같은 문제들을 고려해서 신중하게 프로그래밍 해야 한다.
...
// -> 이느 쪽을 선택해도 별 차이는 없지만, Thread클래스를 상송받으면 다른 클래스를 상속받을 수 없기 때문에
