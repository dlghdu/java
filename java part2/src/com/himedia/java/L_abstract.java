...
// 요약
// - 추상 클래스 : 인스턴스화 할 수 없으며, 공통된 속성이나 메서드를 정의하고,
// 일부 메서든느 자식 클래스에서 구현하도록 강제한다.
// - 추상 메서드 : 선언만 있고, 구현이 없으며, 반드시 자식 클래스에서 오버라이딩 되어야 한다.

// 추상 클래스와 추상 메서드를 활용하면 코드의 구조를 더 명확하게 만들고,
// 하위 클래스에서 필수적으로 구현해야할 메서드를 강제함으로서 코드의 일관성을 유지 할 수 있다.

// [단점]
// 1. 상속의 제약
// 단일 상속의 제약 : 자바는 단일 상속만 지원하므로, 한 클래스가 두 개 이상의 추상 클래스를 상속 받을 수 없다.
// 이는 복잡한 다중 상속 구조를 만들고자 할 때 제약이 될 수 있다.

// 2. 구현의 강제성
// 모든 자식 클래스에서 구현 강제 : 추상 메서드를 상속 받는 모든 자식 클래스는 해당 메서드를 반드시 구현해야 한다.
// 때로는 모든 자식 클래스에서 동일한 메서드 구현이 필요하지 않을 수 있는데,
// 이런 경우에도 모든 자식 클래스에서 해당 메서드로 구현해야 하는 부담이 있다.
