...
// 기존 클래스를 확장하여 새로운 클래스로 정의할 수 있다.

// * 주요특징
// 1. 부모 클래스(슈퍼클래스, superclass)
// 상속을 제공하는 클래스이다. 다른 클래스들이 상속 받을 수 있는 일반적인 속성과 메서드를 정의한다.

// 2. 자식 클래스(서브클래스, subclass)
// 부모 클래스를 상속 받아 확장하는 클래스. 부모 클래스의 모든 속성과 메서드를 물려받으며,
...
// 이떄 부모 클래스의 메서드와 동일한 이름, 반환 타입, 매개변수를 가져야 한다.

// 5. 다형성(Polymorphism)
// 상속을 통해 자식 클래스는 부모 클래스의 형태로 사용도리 수 있으며, 이는 코드의 유연성과 확장성을 높인다.

// * 오버라이딩(Overriding) 단축키 : ctrl+o
...
// 3. 메서드 시그니처
// 오버라이딩 하는 메서드는 부모 클래스에서의 메서드와 동일한 시그니처(이름,매개변수,반환타입)을 가져야한다.
