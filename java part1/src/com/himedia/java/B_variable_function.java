...
// 정수를 저장하는데 사용된다. byte는 이전데이털르 다룰 때, short는 C언어와의 호환을 위해 추가되었다.
// 크기 : byte(1byte-8bit), short(2byte), int(4byte)m long(8byte)
...
// * return
// 1. 값을 변환 : 함수가 어떤 값을 계산하거나 처리한 후, 그 결과를 호출한 코드로 돌려줄 때 사용.
// 2. 함수 종료 : return 문이 실행되면, 해당 함수 즉시 종료가 된다.
...
public class B_variable_function {
    ...
    *  2. 매개변수(Prameters) : 매개변수는 함수가 작업을 수행하는데 필요한 입력값을 전달받는 부분이다.
